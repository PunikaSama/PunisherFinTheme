...
$project = Join-Path $projectRoot "src\PunisherFinTheme\PunisherFinTheme.csproj"
$solution = Join-Path $projectRoot "PunisherFinTheme.slnx"
[xml]$projectXml = Get-Content -LiteralPath $project
$version = [string]$projectXml.Project.PropertyGroup.Version
$artifactRoot = Join-Path $projectRoot "artifacts"
$packageFolder = Join-Path $artifactRoot "PunisherFinTheme_$version"
$packageFile = Join-Path $artifactRoot "PunisherFinTheme_$version.zip"
$output = Join-Path $projectRoot "src\PunisherFinTheme\bin\$Configuration\net10.0"
...
Copy-Item -LiteralPath (Join-Path $output "Jellyfin.Plugin.PunisherFinTheme.dll") -Destination $packageFolder
